...
const Followed = ({posts}) => {
  ...
  return (
    <AuthenticatedLayout
            user={user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Publications des personnes que vous suivez</h2>}
        >
            <Head title="Suivi(e)s" />
            <div className="py-12">
                            ...
                            {posts && posts.data.map(post => (
                                <div className='p-6 m-4 rounded-md border-2 border-gray-300 flex flex-col gap-5' key={post.id}>
                                    <div className='flex items-center justify-between'>
                                        <a href={`/profile/${post.creator.name}`} className={`flex items-center gap-2`}>
